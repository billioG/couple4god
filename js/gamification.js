...
window.sendFlag=async(id)=>{
    ...
    await window.refreshUserProfile(); // Actualizar XP visualmente
    window.showToast("Enviada (+10 XP)"); window.checkNotifications(); window.checkWhiteFlagStatus();
};
window.acceptFlag=async(id)=>{
    ...
    await window.refreshUserProfile(); // Actualizar XP visualmente
    window.showToast("Aceptada (+25 XP)"); window.checkNotifications(); window.checkWhiteFlagStatus();
};
...
window.loadPrayers = async function() {
    ...
    if(items&&items.length>0) items.forEach(i => {
        ...
        else act = sups.length>0 ? `<span style="color:var(--primary); font-size:0.8rem">❤️ Tu pareja te escuchó</span>` : `<span style="color:#666; font-size:0.8rem">Esperando...</span>`;
...
window.supportPrayer = async function(id, btn) {
    ...
    try {
        ...
        if(!arr.includes(window.currentProfile.id)) {
            ...
            // La actualización visual del botón se hace sola por el Realtime en app.js
        }
...
window.loadDeepQuestion = async function() {
    const container = document.getElementById('dynamic-content'); container.innerHTML = '<div class="loader">Cargando...</div>';
    const questions = ["¿Recuerdo feliz?", "¿Qué admiras?", "¿Cuándo te sentiste amado?", "¿Miedo?", "¿Hogar?"];
    ...
    let html = `<div style="padding:0 20px 80px;"><div class="garden-card" style="margin:0 0 20px;"><h3 style="color:#aaa; font-size:0.8rem">PREGUNTA</h3><h2 style="color:white; margin:10px 0;">"${q}"</h2></div><div style="background:var(--card-bg); padding:15px; border-radius:15px; border:1px solid #333;"><textarea id="answer-input" class="input-field" style="height:70px;" placeholder="Respuesta..."></textarea><button onclick="saveSharedContent('answer', '${q}')" class="btn-primary">Compartir (+5 XP)</button></div><h3 style="margin-top:30px;">Respuestas</h3>`;
    if(ans) ans.forEach(a => { html += `<div style="background:#252a35; padding:15px; margin-top:10px; border-radius:10px; border:1px solid ${a.user_id===window.currentProfile.id?'#444':'var(--primary)'}"><small style="color:#888">${a.user_id===window.currentProfile.id?'Tú':'Tu pareja'}:</small><p style="color:#ddd; margin-top:5px;">${a.content}</p></div>`; });
